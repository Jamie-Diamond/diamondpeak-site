...
- **Interval breakdowns**: use Strava lap splits (including auto 5km Garmin/Strava laps) for breakdown — NEVER Intervals.icu auto-detected efforts. For Z2/easy sessions with only auto-distance laps, show those laps (5km splits) or note there are no manual lap markers. For structured BIKE/RUN interval sessions, auto-detected efforts from Intervals.icu are fine to show alongside Strava laps. SWIM rep timings are the exception — ICU auto-detected swim intervals over-extend every rep ~2–4s into the rest (boundaries inferred from velocity_smooth): swim rep durations/paces ALWAYS come from Strava laps (native Garmin button-press boundaries); ICU swim intervals only for per-rep HR (confirmed 11 Jun 2026).
