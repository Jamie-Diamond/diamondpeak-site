#!/bin/bash
# Daily session prescription — W2. Runs via launchd at 06:33 daily.
...
Step 3 — Assemble the readiness dict:
  ...
  hrv_trend_pct: (today HRV − 7d avg HRV) / 7d avg HRV × 100  [if no HRV data, use 0.0]
...
/Users/diamondpeakconsulting/.local/bin/claude -p "$PROMPT" --allowedTools "$TOOLS" 2>>"$HOME/Library/Logs/ClaudeCoach/prescription.log"
