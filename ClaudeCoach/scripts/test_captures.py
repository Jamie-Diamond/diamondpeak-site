...
# --- 7) hours TIER 2 must stand down for fast-path LOGS ---------------------------------
# The fall-through's sharpest edge. looks_like_hours_reply("heat 30") is True, so with the
# Sunday ask outstanding that message used to be swallowed as a possible hours figure (heat
# session lost). Now it carries on to the fast path — so the tier must not ALSO ask "is that
# 30 hours next week?", or one reflex tap sets the week's Load ceiling to 30.
import weekly_availability as W
for logged in ("heat 30", "bath 20", "ankle 3", "pain knee 4", "weight 82.5",
               "css 1:38", "lthr 165", "new ftp 320", "period started", "cycle day 12"):
    check(f"{logged!r} is recognised as a fast-path log",
          B.logs_via_fast_path(logged))
# NOT "82.5 kg": _WEIGHT_RE wants the keyword first ("weight 82.5"), so a bare
# figure-then-unit is not a fast-path log at all — the model handles it. Asserted so the
# gate's coverage is not mistaken for the full set of ways a weight can be logged.
for real in ("7", "12", "12 max, nothing long midweek", "14 hours next week",
             "20, big week", "82.5 kg"):
    check(f"{real!r} is NOT treated as a fast-path log",
          not B.logs_via_fast_path(real))
check("the gate is only needed where the two genuinely overlap",
      W.looks_like_hours_reply("heat 30") and B.logs_via_fast_path("heat 30"))
check("a log with trailing whitespace is still a log", B.logs_via_fast_path("  heat 30 "))
check("empty text is not a log", not B.logs_via_fast_path("") and not B.logs_via_fast_path(None))


# --- 8) honest copy, post-tap -----------------------------------------------------------
