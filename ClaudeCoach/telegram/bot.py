...
def capture_context_note(kind: str, detail: str, question: str = "") -> str:
    ...
    lines = [
        ...
        "",
        "This was a BOOKKEEPING write only. No session, no calendar entry and no plan has "
        "been changed by it. If the athlete's message asks for something to CHANGE, that "
        "change is still outstanding and is yours to make — treat the message as a live "
        "instruction, not as something already handled.",
    ]
    if question:
        lines += ["",
...
def _route_text(token, chat_id, text, athletes, config):
    ...
    for _cap in (_handle_race_capture,      # "I'm racing X on Saturday" -> the race registry
        ...
        capture = _cap(token, chat_id, text, athletes)
        if capture:
            break
