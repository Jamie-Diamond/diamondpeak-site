...
- **Bug logged (4 Jul, rule 111):** long-run progression baseline used a stale "BLOCKED" status note instead of the actual synced 28 Jun run (15.2km Grimaud) — produced a wrong 19.5km target, caught by Jamie's own arithmetic check. Fixed; new persistent rule added (verify baseline against synced data, not status labels).
...
- **CTL/TSB reconciliation & ramp formula (Fri 3 Jul):** 89.1 confirmed correct (87.03+(177−87.03)/42 = 89.15); the earlier 89.4 sighting carried the Ride's *planned* 106 Load before the actual lower 97 synced. Rule of thumb: TSB ≈ −5 × weekly ramp (+2/wk → −10; +4/wk, Jamie's recent ceiling → −20) — explains the −18 to −35 readings across this block.
- **Thu 2 Jul — CSS Swim (COMPLETED):** 33m pool, 2,904m (8×8 lengths, bonus 2 vs planned 6). Rep pace 1:38–1:41/100m, right on CSS. TSS 80 vs planned 58 (+38%). RPE 7.
- **Thu 2 Jul — Z2 Ride 105min (COMPLETED):** Not Z2 — NP 224W/IF 0.75. TSS 97 vs 106 (−8%). No heat dose (24.4°C, just under 25°C gate).
...
- **Heat acclimation:** Score **77%** (77 days to race). No dose added from Thu/Fri/Sat rides — all missed or unconfirmed vs the 25°C gate.
- **W10 plan (29 Jun–5 Jul):** Completed to date 512 (through Thu) + 149 (Sat actual, was 93 planned) = 661. Projected week incl. Sun run (94 planned) = 755 vs ~764 target — now roughly on track (Sat's overrun closed most of the earlier −8% gap).
