...
- **France camp (22–28 Jun) — Day 3 COMPLETE (Thu 25 Jun):** Mon 22 easy run ✓ 49 TSS · Tue 23 swim ✓ 52 TSS · Wed 24 Z2 run ✓ 51 TSS · **Thu 25 Z2 ride ✓ 119 TSS** · Thu PM OW swim ~51 TSS (pending) · Fri 26 long ride 183 · Sat 27 endurance ride IF ~0.71 ~105 TSS · Sun 28 long run + OW swim ~117. **Projected week 727 TSS** (genuine build — floor 608 maintenance). Heat TSS undersells real stress.
- **Thu 25 Jun — Z2 ride 2h (COMPLETED):** 151 min, NP 206W (IF 0.69), 740m gain, 119 TSS vs 92 planned (+29%). NOT clean Z2 — zone split: 71% Z1–Z2 / 14% Z3 / 12% Z4 / 3% Z5 (climbing drove it). ERA5 32→35.5°C (hottest session to date). Nutrition 86g/hr carbs (216g total), ~1000ml/hr fluid (~2.5L). Ankle not relevant for bike.
- **Wed 24 Jun — Z2 run (COMPLETED, EASY):** 8.91km, 51 TSS, GAP 5:10/km, avg HR 143 vs 150 ceiling = EASY. ERA5 30.9–32°C (RH 49%). Ankle 3/10 first half → 1–2/10 second half (highest during-run score since physio clearance, still ≤5/10 tolerance). Next: open at 5:15–5:20 GAP in heat.
- **Tue 23 Jun — Speed/technique swim (COMPLETED, last pool before France OW week):** 2,000m, 52 TSS. 4×100m drill WITH paddles (catch/DPS — no speed target per rule 78); 8×100m paddle-OFF avg 1:38 vs TGT 1:36–1:38 (held to last rep, final 3 at 1:36); 2×200m pull buoy at CSS 1:41/1:40.
- **Mon 22 Jun — Easy run (COMPLETED, EASY):** 7.62km, 49 TSS, GAP 4:53/km, avg HR 141 vs 146 ceiling. ERA5 ~28°C.
- **Ankle:** ATFL + CFL partial tear. Latest: 3/10 peak during 24 Jun France run, settled 1–2/10 second half. Protocol: ≤5/10 OK if resolves by next AM; >5/10 = stop. Thu AM score not given (bike day — not relevant).
- **Heat acclimation:** Live score **39% ↑** (heat_accl.py — 5 exposures ≥28°C since 21 Jun; May Marseille block largely decayed). Morning card 25 Jun missed it (card fired ~06:30, before 07:08 deploy) — shows from 26 Jun. ⚠️ Coach error 25 Jun: estimated 65–70% instead of running script; rule 82 added.
- **CP vs FTP (25 Jun):** eFTP 297W = working metric for IM; CP+W′ passively populated from power curve; no structured test needed; race pacing far below threshold makes the gap irrelevant.
- **Camp plan remaining:** Thu PM OW swim 50 min (~51 TSS) · Fri long ride 4h Z2 (183 TSS — unchanged) · Sat endurance ride 2h IF ~0.71 (~105 TSS, intensity up not duration) · Sun long run ~15km (~92 TSS) + OW swim (~25 TSS). Sat change: let terrain/IF sit at 0.71 matching today's climbing profile, not a pure Z2 spin.
...
- **Bug list (25 Jun — 13 logged):** Pending code: #5 replan guard, #8 bug list maintenance, #11 voice reply. Rules-corrected: #6→77, #7→74, #9→75, #10→76, #13→81, heat score→82. Deployed: #12 heat tracker 07:08 25 Jun.
