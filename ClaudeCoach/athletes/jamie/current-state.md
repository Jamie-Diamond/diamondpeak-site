...
- **Sun 5 Jul readiness — reconfirmed mid-day ("How's my recovery"):** HRV 34 (tied lowest in 14d; range 34–50, avg ~40), RHR 57 (near top of 51–58 range, above ~54 baseline), sleep 77 (not the limiter). TSB −30.3, ATL 126.2 — all three signals agree, still not recovered from Fri/Sat back-to-back big days. AM call stands: cap today's long run near 12km, hold GAP easy, don't chase splits. Injury pain score asked twice (AM + this check), not yet answered.
- **Today's long run (Sun 5 Jul):** base plan was 82min/16.9km/94 Load (event 119142487, corrected from a stale 13.5km baseline to the actual 15.2km Grimaud run ×1.10–1.15) — now further capped to ~12km per recovery signals. Running late afternoon (Jamie's choice) for max recovery buffer, a heat-accl opportunity, and a cleaner ~60g/hr run-fuelling test (rule 112) on a fed stomach — outcome still pending, not yet completed/synced.
- **Sat 4 Jul — Wandsworth Road Cycling / Sweetspot Ride Modified 2×15 (COMPLETED):** 149 Load vs 97 planned (+54%; ride ran ~210min vs 110 planned). NP 198W/IF 0.67/VI 1.41, hilly (874m gain, 29% coasting), 81% Z1–Z2, avg HR 118, pedalling-only decoupling −2.6% (whole-ride 11.6% is a coasting artifact). Nutrition ≈200g carb (Maurten 320 + 2 HIGH5 Caffeine Hit scoops + 2×SiS choco fudge bar + 1 PF30, ~57g/hr — duration-diluted, total grams solid) + ~2.8L fluid + PH1500. RPE 4 (base) but 350–400W surges (118–135% eFTP) at RPE 9/10 — covers the periodic hard-effort need, no structured intensity needed soon. 7 PRs. No heat dose (ERA5 ~20–25°C, below gate).
- **Fri 3 Jul — Long Z2 Ride (COMPLETED):** ON TARGET. NP 206W/IF 0.69, HR 95% Z1+Z2, pedalling-only decoupling 4.4% (whole-ride 8.7% coasting artifact). TSS 223 vs 232 planned (−4%). 14 PRs. No heat dose (ERA5 peak 20.6°C). RPE not numeric — Jamie: "felt pretty dead after that/now." Combined with Sat's ride, pushed TSB to the heaviest point of this block (−35.3 Sat evening; −30.3 Sun AM).
- **CTL/TSB ramp formula (rule 116)** explained to Jamie: TSB ≈ −5 × weekly ramp — clarifies why he's been living at −18 to −35 while ramping near the +4/wk cap.
- **Nutrition focus (rule 112):** bike carb tolerance proven (90–100g/hr demonstrated) — stop chasing it every ride. Open question is the run: can Jamie hit ~60g/hr while running; today's run is the test.
- **Weight:** no numeric figure logged since 85.3kg on 30 Jun (vs 79kg target, +6.3kg) — chart/photo sent Fri, Sat and Sun mornings but Jamie hasn't replied with a number.
- **Ankle:** ATFL + CFL partial tear. Latest score 2/10 (Wed 1 Jul, within 5/10 tolerance). Protocol unchanged: ≤5/10 = continue; >5/10 = stop; not resolved next AM = no run.
- **Heat acclimation:** Score 77%. No dose added from Thu/Fri/Sat rides — all below the 25°C/60min gate.
- **W10 plan (29 Jun–5 Jul):** 512 completed (through Thu) + 149 (Sat actual) + 94 (Sun original plan) = 755 vs ~764 target (~−1%) as of Sat evening — today's shortened ~12km run will trim this further, not yet finalised.
- **Open action:** Aero helmet — overdue since 30 Jun, re-flagged repeatedly across morning check-ins, still unconfirmed.
- **Strava description generator (fixed 2026-07-03):** patched for watch-sensor temp and false "heat" labelling. Open commitment: re-run Wed 1 Jul's description with full prose once Haiku 401 auth blip clears (no due date given).
- **France trip #2** (holiday, not a "camp") starts 10 Jul (5 days out) — full swim/bike/run + heat access, peak build window.
